...
static const uint8_t font8x8_basic[128][8]; // predeclare it
...
void drawchar(uint8_t ch, uint64_t x, uint64_t y, uint32_t fgcolor, uint32_t bgcolor) {
  ...
  if (ch < 0 || ch > 0x7F) { // check ascii 0-127 (null char to delete char)
    return;
  }
  
  for (uint64_t ycoord = 0; ycoord < 8; ycoord++) { // 8x8 chars
    for (uint64_t xcoord = 0; xcoord < 8; xcoord++) {
      if (font8x8_basic[ch][ycoord] & (1 << xcoord)) { // bit 1 (foreground)
...
// TODO BIG FONT DEFINITION BELOW
